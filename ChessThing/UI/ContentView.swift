...
struct CheckersView: View {
        ...
        VStack(spacing: 12) {
            ...
            if game.winner != nil {
                if game.winner == .white {
                    Text("Winner! White wins!").font(.subheadline).foregroundStyle(.secondary)
                }else if game.winner == .black{
                    Text("Winner! Black wins!").font(.subheadline).foregroundStyle(.secondary)
                }
            }
        }.padding()
    }
    ...
    private func handleTap(row: Int, col: Int) {
        ...
        if let selected = game.selectedSquare, let sourcePiece = game.board[selected.row][selected.col].piece {
